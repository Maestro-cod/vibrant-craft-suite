...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([list(), getStats()])
      .then(([u, s]) => {
        setUsers(u.users as Row[]);
        setStats(s);
      })
      .catch((e) => toast.error(e?.message ?? "Failed to load"));
  }, [profile, loading, list, getStats, nav]);
    ...
    <AppShell>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 pt-8 space-y-6">
        <div className="flex items-center gap-3">
          <div className="size-12 rounded-xl bg-gradient-brand grid place-items-center glow-cyan">
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <GlassCard className="p-4">
              ...
              <div className="text-2xl font-bold mt-1">{stats.generations}</div>
            </GlassCard>
            <GlassCard className="p-4">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <div className="text-2xl font-bold mt-1">
                {(stats.byPlan.pro ?? 0) + (stats.byPlan.elite ?? 0)}
              </div>
            </GlassCard>
            <GlassCard className="p-4">
              <div className="text-xs text-muted-foreground">Free</div>
              <div className="text-2xl font-bold mt-1">{stats.byPlan.free ?? 0}</div>
            </GlassCard>
          </div>
        )}
              ...
              {filtered.map((u) => (
                <tr key={u.id} className="border-t border-white/5">
                  ...
                  <td className="px-4 py-3">
                    <select
                      value={u.plan}
                      onChange={(e) => updatePlan(u.id, e.target.value as Row["plan"])}
                      className="px-2 py-1 rounded-md glass-strong"
                    >
                      {(["free", "basic", "pro", "elite"] as const).map((p) => (
                    ...
                    {u.unlimited && (
                      <span className="ml-2 text-[10px] text-gradient font-bold">∞</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (!Number.isNaN(v) && v !== u.credits) updateCredits(u.id, v);
                      }}
