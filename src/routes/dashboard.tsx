...
const TILES = [
  {
    to: "/video",
    label: "New Video",
    desc: "Cinematic clips, 9:16 / 16:9 / 1:1",
    icon: Video,
    span: "sm:col-span-2",
    glow: "glow-cyan",
  },
  ...
  {
    to: "/voice",
    label: "New Voice-Over",
    desc: "ElevenLabs voices",
    icon: Mic,
    span: "",
    glow: "glow-cyan",
  },
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 pt-8 space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {profile?.unlimited
              ...
              : `${profile?.credits ?? 0} credits • ${profile?.plan ?? "free"} plan`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            {
              label: profile?.unlimited ? "Plan" : "Credits left",
              value: profile?.unlimited ? "Elite ∞" : (profile?.credits ?? 0),
              icon: Coins,
            },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {TILES.map(({ to, label, desc, icon: Icon, span, glow }) => (
            <Link
              key={to}
              to={to}
              className={`glass rounded-2xl p-6 lift gradient-border group ${span}`}
            >
              <div className="flex items-start justify-between">
                <div
                  className={`size-12 rounded-xl bg-gradient-brand grid place-items-center group-hover:${glow} transition`}
                >
                ...
                return (
                  <GlassCard key={g.id} className="p-5 flex flex-col gap-3 lift">
                    ...
                    <div className="flex gap-2 mt-auto pt-2">
                      <button
                        onClick={() => download(g)}
                        className="flex-1 py-2 rounded-lg bg-gradient-brand text-primary-foreground text-sm font-medium flex items-center justify-center gap-1.5 hover:opacity-90"
                      >
                        <Download className="size-3.5" /> Download
                      </button>
                      <button
                        onClick={() => remove(g.id)}
                        className="p-2 rounded-lg glass-strong hover:bg-white/10"
                        aria-label="Delete"
                      >
