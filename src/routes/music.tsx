...
import { generateMusic } from "@/lib/audio.functions";
...
export const Route = createFileRoute("/music")({
  head: () => ({
    meta: [
      { title: "AI Music — HyperPost AI" },
      { name: "description", content: "Generate original music & soundscapes from a brief." },
    ],
  }),
  component: MusicPage,
});
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl px-4 sm:px-6 pt-8 space-y-6">
        <div className="flex items-center gap-3">
          <div className="size-12 rounded-xl bg-gradient-brand grid place-items-center glow-violet">
            <Music className="size-5 text-background" />
          </div>
          <div>
            <h1 className="text-3xl font-bold">AI Music</h1>
            <p className="text-muted-foreground text-sm">
              Describe a mood, genre or reference. Get a track up to 22 seconds.
            </p>
          </div>
        </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {PRESETS.map((p) => (
                <button
                  key={p}
                  onClick={() => setBrief(p)}
                  className="px-3 py-1.5 rounded-full glass-strong text-xs hover:bg-white/10"
                >
                  {p.split(",")[0]}
                </button>
              ))}
