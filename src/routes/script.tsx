...
export const Route = createFileRoute("/script")({
  head: () => ({
    meta: [
      { title: "Script Generator — HyperPost AI" },
      { name: "description", content: "Generate viral short-form video scripts." },
    ],
  }),
...
function ScriptPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      await supabase
        .from("generations")
        .insert({ user_id: user.id, type: "script", prompt: topic, output_text: text });
